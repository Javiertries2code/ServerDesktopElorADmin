	...
	public DataListener<MessageInput> userLogin() {
		return ((client, data, ackSender) -> {
			...
			if (jsonObject.has("nameValuePairs")) {
				...
				System.err.println(
						"Error " + ErrorCode.INVALID_INPUT.getCode() + ": " + ErrorCode.INVALID_INPUT.getMessage());
...
//				Map<String, Object> userStatus = mL.validUser(email, password);
			...
			case "registered":
				...
				try {
					...
					// Sending both thecorrect message, and encripted, for the sake of meeting
					// requirements
					...
					try {
						answerMessage = desEncoder.encoder(answerMessage, null);
					} catch (Exception e) {
						System.err.println("Error " + ErrorCode.ENCRYPTION_ERROR.getCode() + ": "
								+ ErrorCode.ENCRYPTION_ERROR.getMessage());

					}

					MessageOutput cryptedDESMessageOutput = new MessageOutput(answerMessage);
					client.sendEvent(Events.ON_SERVER_SENDING_ENCRYPTED_DES.value, cryptedDESMessageOutput);
	...
	public DataListener<MessageInput> resetPassword() {
		return ((client, data, ackSender) -> {
			...
			if (jsonObject.has("nameValuePairs")) {
				...
				System.err.println(
						"Error " + ErrorCode.INVALID_INPUT.getCode() + ": " + ErrorCode.INVALID_INPUT.getMessage());
				client.sendEvent(Events.ON_RESET_PASSWORD_FAILER.value,
						new MessageOutput("Invalid request: email is missing"));
			}
			...
			// Send response back to the client, i guess, showing the response was sent and
			// delivering it to login, should do enough
...
////////////////////////////////////////////////////////////////////
	public DataListener<MessageInput> testingEncription() {
		/*
		 * OOOki doki, if everything happens as i wish ut does... it will bounce back
		 * the encrypted message, that as it is a valid user should contain this data
		 * :-P
		 * 
		 */
		return (client, data, ackSender) -> {
...
////////////////////////////////////////////////////////////
	...
	public DataListener<MessageInput> testingEncriptionDES() {
		/*
		 * OOOki doki, if everything happens as i wish ut does... it will bounce back
		 * the encrypted message, that as it is a valid user should contain this data
		 * :-P
		...
		return (client, data, ackSender) -> {
			...
			// System.out.println(message + " the mensaje ");
			DESEncoder codeEncode = new DESEncoder();
//Code me... code youuuu!!! coding forever...
			// message = codeEncode.decrypt(message);
				Map mymessage = new HashMap<String, byte[]>();
				mymessage.put(message, null);
			// it seems i had to cast, fuck me...
			try {
				message = codeEncode.decoder(mymessage, null);
			} catch (Exception e) {
				System.err.println("Error " + ErrorCode.ENCRYPTION_ERROR.getCode() + ": "
						+ ErrorCode.ENCRYPTION_ERROR.getMessage());				
				//e.printStackTrace();
